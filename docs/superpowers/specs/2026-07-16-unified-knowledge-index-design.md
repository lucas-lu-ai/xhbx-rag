...
1. `metadata.title/category/scenario/tags`，以及绩优案例已有的 `tag_paths`、`business_*`、`sales_stages`、`customer_*`、`product_categories`、`objection_types`、`compliance_risks`、`knowledge_type` 等结构化字段。
...
现有 977 个培训资料文件中的 chunk 使用 `chunk_type=knowledge_entry`，当前 `RagChunk` 合同不接受该类型。
...
7. 同一来源类型内重复的 chunk_id 视为同一逻辑知识的重复抽取版本：按稳定路径保留首条，后续记录为 `deduplicated_chunk_id` warning；跨来源类型的 ID 冲突、无效 JSON、未分类 chunk 或不支持路径返回非零。空文件作为显式 warning 记录并跳过。

报告至少包含：

- 输入文件数、有效文件数、空文件数。
- 输入 chunk 数、可入库唯一 chunk 数、去重 chunk 数以及按 `source_kind` 的数量。
- `primary_domain` 与 `domain_tags` 分布。
- 多标签 chunk 数量。
- 无效记录、未分类记录、同来源去重和跨来源 ID 冲突的文件及行号。
- 每个领域的代表性样本和规则命中信息。
...
3. 按 `batch-size` 分批调用 embedding，不能把当前 18,670 条唯一文本一次性发送。
...
- 同一来源类型内 chunk_id 重复：稳定保留首条，报告全部后续冲突来源为 `deduplicated_chunk_id` warning。
- 培训资料与绩优案例之间 chunk_id 重复：报告双方位置，规范化失败。
...
- 递归发现两种文件模式且不重复。
- 空文件和同来源重复版本被明确报告并跳过；坏 JSON、跨来源 ID 冲突、未分类返回非零并出报告。
...
- 来源类型数量与目录规则一致。
- 报告明确呈现 18,930 条输入、260 条同来源重复版本和 18,670 条唯一可入库记录，无静默丢弃。
