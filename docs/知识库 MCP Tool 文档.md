...
通常选择 1 至 3 个最相关领域；跨领域问题可以多选。无法可靠判断时传入全部七类，
等价于在领域维度覆盖当前完整知识体系。
...
| `primaryDomains` | List\<string\> | 是 | 1 至 7 个固定一级领域；去除首尾空白、自动去重并保留首次出现顺序 |
...
`primaryDomains` 为空、不是数组、含非字符串项、含体系外标签或超过七项时，返回包装后的
`10004`：
