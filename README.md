...
- `search_knowledge(query)`：MCP 侧只接收原始问题，不调用 chat/completions 做 query understanding；内部使用固定候选数执行 embedding → 向量 + 关键词混合召回 → RRF 融合 → rerank，返回证据 chunk（含知识类型、原文引用与定位）。
