...
- `kb_search_knowledge(query, primaryDomains, knowledgeTypes=None, retrievalMode="HYBRID", hybridWeights=None, topK=10, includeDetails=false)`：在统一 collection 中按 `primary_domain` 硬过滤并检索。`primaryDomains` 必填，只允许 `产品知识`、`合规与风控`、`销售技能`、`客户经营`、`行业与公司`、`个人成长`、`组织发展`；支持多选、自动去重，非法输入返回包装后的 `10004`。MCP 不调用 chat/completions、不自动识别 query 领域，也不按 `source_kind` 区分培训资料与绩优案例。当前仅支持 `HYBRID`，`SPARSE` / `VECTOR` 返回 `10004`；`topK` 默认 10、最大 50。
