...
服务对齐《知识库 MCP Tool 文档》平台契约（见 `docs/知识库 MCP Tool 文档.md`）：默认工具返回统一包装为 `McpResponse`，成功为 `{"success": true, "data": ...}`，失败为 `{"success": false, "errorCode": "...", "errorMessage": "..."}`（错误码 `10004` 参数错误、`500` 系统内部错误）。默认只提供一个工具：
...
- `kb_search_knowledge(query, primaryDomains, knowledgeTypes=None, retrievalMode="HYBRID", hybridWeights=None, topK=10, includeDetails=false)`：`primaryDomains` 是必传数组。能够匹配现有一级体系时，从 `产品知识`、`合规与风控`、`销售技能`、`客户经营`、`行业与公司`、`个人成长`、`组织发展` 中传入一个或多个领域，MCP 在统一 collection 中按 `primary_domain` 硬过滤；无法匹配现有体系时传入 `[]`，MCP 不生成领域过滤并查询全部文档，包括未标注或异常领域的历史数据。数组支持自动去重，非法输入返回包装后的 `10004`。MCP 不调用 chat/completions、不自动识别 query 领域，也不按 `source_kind` 区分培训资料与绩优案例。当前仅支持 `HYBRID`，`SPARSE` / `VECTOR` 返回 `10004`；`topK` 默认 10、最大 50。

`includeDetails` 默认为 `false`，每条结果返回四字段精简切片：`docId`
取第一条 citation 的 `source_id`，缺失时为空字符串；`knowledgeType` 固定为
`SLICE`，`title` 固定为 `切片`，`content` 为完整正文。`data` 按检索排序返回，
数量最多为 `topK`；结果不足时按实际数量返回：
...
领域元数据仅在 `includeDetails=true` 的完整模式中返回；该模式保留完整切片结构，
并在结果顶层返回 `primaryDomain` 和 `domainTags`。
...
- `MCP_TOOL_PROFILE=kb`：默认，只暴露 `kb_search_knowledge`
...
原始数据文件应放在项目 `data/` 目录下。回答下方“知识引用”只显示模型实际引用的 evidence chunks，并使用连续引用序号；点击后，右侧“引用明细”会展示知识名称、`source_path`、文件位置、原文摘录和定位置信心，方便核对回答依据。点击“在 Finder 中显示文件”只会打开 `data/` 目录内的本地文件；第一版不会尝试把 Word/PPT/PDF 精确跳转到段落或页内锚点。
