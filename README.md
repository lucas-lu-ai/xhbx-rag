...
- `search_knowledge(query, chunk_types=None, stage="", case_name="")`：MCP 侧不调用 chat/completions 做 query understanding；内部使用固定候选数执行 embedding → 向量 + 关键词混合召回 → RRF 融合 → rerank，返回证据 chunk（含知识类型、原文引用与定位）。`chunk_types`、`stage`、`case_name` 都是可选精确过滤条件，不传或传空时不参与过滤。
- `list_filter_options()`：返回当前索引中可用于 `search_knowledge` 的合法过滤值，包括 `chunk_types`（含中文 label）、`stages`、`case_names`。外部客户端应先读取这些值，只在用户明确限定知识类型、阶段或案例名时再传过滤参数；不确定时不要传过滤，避免误过滤。
