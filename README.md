...
- `kb_search_knowledge(query, kbId, knowledgeTypes=None, retrievalMode="HYBRID", hybridWeights=None, topK=10, includeDetails=false)`：在指定 `kbId` 知识库内检索。MCP 侧不调用 chat/completions 做 query understanding；当前实现仅支持 `HYBRID`，`SPARSE` / `VECTOR` 暂返回参数错误 `10004`。完整模式以 `knowledgeType="SLICE"` 输出：`slice.content` 为截断预览、`slice.fullContent` 为全文、`slice.sliceType` 保留原业务类型（客户旅程 / 销售策略 / 场景话术 / 异议处理 / 培训课程）、`slice.citations` 为原文引用与定位（平台契约外的扩展字段）。`topK` 默认 10、最大 50。

`includeDetails` 默认为 `false`，每条结果返回四字段精简切片摘要：`docId`
取第一条 citation 的 `source_id`，缺失时为空字符串；`knowledgeType` 固定为
`SLICE`，`title` 固定为 `切片`，`content` 为完整正文（缺失时为空字符串）：
...
`includeDetails` 设为 `true` 时返回完整切片结构。
