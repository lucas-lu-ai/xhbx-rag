...
`includeDetails` 默认为 `false`，每条结果返回六字段精简切片：`docId`
取第一条 citation 的 `source_id`，缺失时为空字符串；`knowledgeType` 固定为
`SLICE`，`title` 固定为 `切片`，`content` 为完整正文，`primaryDomain` 为主领域，
`domainTags` 为一级领域标签数组：
...
`includeDetails` 设为 `true` 时返回完整切片结构，并在结果顶层返回同样的 `primaryDomain` 和 `domainTags`。
